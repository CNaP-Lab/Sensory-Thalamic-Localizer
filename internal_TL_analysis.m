function obj = internal_TL_analysis(varargin)
    ...
    addParameter(p, 'trim', false, @islogical);  % Default to not trimming
    addParameter(p, 'nrmv', 0, @isnumeric);      % Default number of volumes to remove
    ...
    if ~(trim && ~isempty(nrmv))
        error('If input field ''trim'' is provided as true then number of volumes to remove ''nrmv'' must also be provided.')
    end
    ...
    if isempty(BOLD_fMRI_Smoothed_data) && smoothingFWHM ~= -1
        disp("Will smoothen and save to obj.sMNI_run")
        obj = TLsmoothVols(obj, smoothingFWHM);
        disp([obj.sMNI_run]);
    elseif ~isempty(BOLD_fMRI_Smoothed_data) && smoothingFWHM == -1
        obj.sMNI_run = BOLD_fMRI_Smoothed_data;
        % The condition below exists because the variable stMNI_run is used here on 
        % but if a trimmed volume is provided to begin with then we set the smoothed data to stMNI_run,
        % and if not provided then the TLsmoothVols function writes into the stMNI_run to maintain this logic.
        if ~trim 
            obj.stMNI_run = BOLD_fMRI_Smoothed_data;
        end
    else
        % Error if neither smoothed data nor smoothing parameter is provided
        % Ideally should not reach here as there are exhaustive checks before
        error('Either smoothed data must be provided or a smoothing FWHM must be specified.');
    end

    if trim
        if nrmv > 0
            disp(['Trimming ', num2str(nrmv), ' volumes from each timeseries and Motion Parameters']);
            obj = customtrim(obj, nrmv);
            disp('Done Trimming!')
        else
            error('Number of volumes to remove must be greater than zero when trimming is enabled.');
        end
    end
